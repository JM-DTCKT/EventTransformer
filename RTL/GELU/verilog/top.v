...
//             내부는 Residual + PWL fixed-point (Q5.10) 로 처리
//
//    x_fp32 ─▶[FP32→Q5.10]─▶ gelu_pwl(core, Q5.10, latency 3) ─▶[Q5.10→FP32]─▶ y_fp32
//              └ Stage A ┘                                        └ Stage E ┘
//
//  Stage A(FP32→Q5.10), Stage E(Q5.10→FP32) 는 조합 변환기다.
...
module top #(
    parameter W       = 16,   // 내부 고정소수점 폭 (Q5.10)
    ...
    // ---------------- Stage A : FP32 -> Q5.10 (조합) ----------------
    wire signed [W-1:0] x_fix_c;
    fp32_to_q510 u_in (.f(x_fp32), .q(x_fix_c));
    ...
    gelu_pwl #(.W(W), .QF(10), .FR(14)) u_core (
        .clk(clk), .rst_n(rst_n),
        .in_valid(va), .x(x_fix),
        .out_valid(cov), .y(y_fix)
    );

    // ---------------- Stage E : Q5.10 -> FP32 (조합) ----------------
    wire [31:0] y_fp32_c;
    q510_to_fp32 u_out (.q(y_fix), .f(y_fp32_c));
