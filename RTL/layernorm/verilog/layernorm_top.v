...
module layernorm_top #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (recv_done) slot_state[slot_recv] <= SL_RECVD;    // S1 -> S2
    ...
    assign recv_done = recv_hs & recv_last;
    ...
    wire [SQW-1:0]        x_cvt_sq   [0:LANE-1];
    generate
        for (gl = 0; gl < LANE; gl = gl + 1) begin : g_rx
            ...
            wire signed [2*IW-1:0] sq_full = x_cvt[gl] * x_cvt[gl];
            assign x_cvt_sq[gl] = {{(SQW-(2*IW-1)){1'b0}}, sq_full[2*IW-2:0]};
        end
    endgenerate
    ...
    always @(posedge clk) begin
        if (recv_hs)
            for (i = 0; i < LANE; i = i + 1) begin
                sum_x[slot_recv][i] <= recv_first
                    ? {{(SXW-IW){x_cvt[i][IW-1]}}, x_cvt[i]}
                    : sum_x[slot_recv][i] + {{(SXW-IW){x_cvt[i][IW-1]}}, x_cvt[i]};
                sum_x2[x2_wslot][i]   <= recv_first ? x_cvt_sq[i]
                                                : sum_x2[x2_wslot][i] + x_cvt_sq[i];
            end
    end
