...
`fpga_nl` 의 `FFN_Engine` 은 명령어 12개짜리였는데도 위상·주소 버그가 5개 나왔습니다.
EvT 는 타임스텝 하나가 **명령어 40개 남짓**이고 attention 3블록 x head 4 가 들어갑니다.
...
FMT_INT8, FMT_Q411, FMT_BF16, FMT_Q69 = 0, 1, 2, 3
OP_GEMM, OP_LN, OP_SMAX, OP_RES, OP_MEAN, OP_ARGMAX, OP_POS = 0, 1, 2, 3, 4, 5, 6
OP_NAME = {OP_GEMM: 'GEMM', OP_LN: 'LN', OP_SMAX: 'SMAX', OP_RES: 'RES',
        OP_MEAN: 'MEAN', OP_ARGMAX: 'ARGM', OP_POS: 'POS'}
FMT_NAME = {FMT_INT8: 'int8', FMT_Q411: 'Q4.11+GELU', FMT_BF16: 'bf16', FMT_Q69: 'Q6.9'}
...
SEQ5_SCALE = 0.019990921020507812
...
def fx_scale(x):
    """scale → round(scale * 2^RSH) (16비트에 들어가야 함)"""
...
def build(n_tok, cfg_dir=None):
    ...
    베이스 주소가 매번 바뀌어 **명령어 프로그램을 타임스텝마다 다시 만들어야** 합니다
    ...
    def gemm(name, layer, M, K, Nout, a_base, b_off, cons, sh, out,
             rq=None, act=0, gpb=None, gsh=0, note='', row0=0):
        # shift / RQ_BASE / W 베이스는 **manifest 에서** 옵니다 (`sh`/`rq` 는 덮어쓰기용).
        ...
        if rq is None:
            rq = (RQB.get(layer) or 0) + row0     # in_proj 밴드는 행 오프셋만큼
        S.append(dict(kind=OP_GEMM, name=name, layer=layer, M=M, K=K, NOUT=Nout,
                      AIN=a_base, BIN=CFG['w_base'][layer] + b_off,
                      AOUT=out, FMT=cons, SHIFT=sh,
                      RQ_BASE=rq, ACT=act, GRQ_BASE=gpb, SHIFT2=gsh, note=note))

    def ln(name, mod, M, a_in, a_out, sh, note=''):
        # LayerNorm 은 **두 메모리**를 읽습니다 — Affine_Mem(gamma/beta, 특징별)과
        # Requant_Mem(Q4.11→int8 재양자화 스칼라 1개). 베이스가 달라서 필드도 둘입니다:
        #   RQ_BASE = 재양자화 스칼라 자리   OSTR = Affine_Mem 베이스
        # (처음엔 RQ_BASE 하나로 둘 다 가리키게 해 두었는데, 서로 다른 메모리라
        #  같은 값일 수가 없습니다.)
        # SHIFT2 는 LN 에서 안 쓰던 칸이라 **코어의 고정소수점 창**(in_shift)을
        # 실어 보냅니다. 6비트 signed 로 인코딩합니다.
        S.append(dict(kind=OP_LN, name=name, layer=mod, M=M, K=E, NOUT=E,
                      AIN=a_in, AOUT=a_out,
                      SHIFT=sh if sh else CFG['ln_shift'][mod],
                      SHIFT2=CFG['ln_xsh'][mod] & 0x3F,
                      RQ_BASE=CFG['ln_rq'][mod], OSTR=CFG['af_base'][mod], note=note))
    ...
    S.append(dict(kind=OP_POS, name='pos_gather', M=n_tok, K=64, NOUT=64,
    ...
    gemm('event_projection', 'backbone.event_projection.seq_init.0',
         n_tok, 144, 96, R['X'], 0, FMT_Q411, None, R['PIN'],
    ...
    gemm('preproc', 'backbone.preproc_block_events.seq_init.0',
         n_tok, 160, E, R['PIN'], 0, FMT_Q411, None, R['PRE'],
         note='160→128, Q4.11→GELU→int8')
    gemm('proc_ev.1', 'backbone.proc_event_blocks.0.seq_init.1',
         n_tok, E, E, R['PRE'], 0, FMT_INT8, None, R['EV1'], act=1,
         note='ReLU')
    gemm('proc_ev.4', 'backbone.proc_event_blocks.0.seq_init.4',
         n_tok, E, E, R['EV1'], 0, FMT_INT8, None, R['EV'], act=1,
         note='ReLU, 이후 +x (add_x_input)')
    S.append(dict(kind=OP_RES, name='proc_ev.res', M=n_tok, K=E,
    ...
    for bi, blk in enumerate(BLOCKS):
        ...
        # 명령어 하나에 출력 하나라는 단순한 모델을 택합니다.
        gemm(f'{short}.in_proj.Q', f'{blk}.attention', LAT, E, E,
             R['LN1'], 0, FMT_INT8, None, R['Q'],
        ...
        for h in range(HEADS):
            ...
            for mt in range(QT):
                S.append(dict(kind=OP_GEMM, name=f'{short}.av.h{h}.t{mt}',
                              ...
                              FMT=FMT_INT8,
                              RQ_BASE=ARQ[blk]['av'][0], SHIFT=ARQ[blk]['av'][2],
                              note='결과를 채널순으로 이어붙임'))

        gemm(f'{short}.out_proj', f'{blk}.attention.out_proj', LAT, E, E,
             R['CTX'], 0, FMT_BF16, None, R['ZATT'], note='→ bf16')
        S.append(dict(kind=OP_RES, name=f'{short}.res_att', M=LAT, K=E,
        ...
        gemm(f'{short}.linear1', f'{blk}.linear1', LAT, E, E,
             R['LNA'], 0, FMT_Q411, None, R['FFN'], note='GELU')
        ...
        gemm(f'{short}.linear3', f'{blk}.linear3', LAT, E, E,
             R['FFN'], 0, FMT_BF16, None, R['Z'], note='→ bf16')
        S.append(dict(kind=OP_RES, name=f'{short}.res_ffn', M=LAT, K=E,
                      AIN=R['ZATT'], AOUT=R['Z'], note='+ z_att'))

    # 타임스텝 끝 : latent_vectors += z
    S.append(dict(kind=OP_RES, name='latent.acc', M=LAT, K=E,
    ...
    tail.append(dict(kind=OP_LN, name='embs.ln', layer=_eln,
                     M=LAT, K=E, NOUT=E, AIN=R['LATV'], AOUT=R['LNA'],
                     SHIFT=CFG['ln_shift'][_eln], RQ_BASE=CFG['ln_rq'][_eln],
                     SHIFT2=CFG['ln_xsh'][_eln] & 0x3F,
                     OSTR=CFG['af_base'][_eln]))
    tail.append(dict(kind=OP_GEMM, name='embs.linear1',
                     layer='backbone.proc_embs_block.linear1', M=LAT, K=E, NOUT=E,
                     AIN=R['LNA'], BIN=CFG['w_base']['backbone.proc_embs_block.linear1'],
                     AOUT=R['FFN'], FMT=FMT_INT8, SHIFT=None,
                     ACT=1, note='ReLU'))
    # 평균의 나누기(1/96)와 다음 GEMM 격자로의 재양자화는 **곱수 하나**에
    # 접혀 레이어 뒤 빈 칸(`RQ_BASE + NOUT`)에 있습니다 — GELU 스칼라와 같은 자리.
    _mln = CFG['mean_layer']
    tail.append(dict(kind=OP_MEAN, name='embs.gap', layer=_mln, M=LAT, K=E, NOUT=E,
                     AIN=R['FFN'], AOUT=R['LNA'], RQ_BASE=CFG['rq_base'][_mln],
                     SHIFT2=CFG['gelu_shift'][_mln],
                     note='latent 96개 평균 → 1행 (레인 0)'))
    tail.append(dict(kind=OP_GEMM, name='clf.linear_1', layer='models_clf.0.linear_1',
                     M=1, K=E, NOUT=E, AIN=R['LNA'],
                     BIN=CFG['w_base']['models_clf.0.linear_1'], AOUT=R['FFN'],
                     FMT=FMT_INT8, SHIFT=None, ACT=1, note='ReLU'))
    tail.append(dict(kind=OP_ARGMAX, name='clf.linear_2', layer='models_clf.0.linear_2',
                     M=1, K=E, NOUT=NCLS, AIN=R['FFN'],
                     BIN=CFG['w_base']['models_clf.0.linear_2'],
                     RQ_BASE=RQB['models_clf.0.linear_2'],
...
def selfcheck(S, tail, arena, R, dims, aw_a):
    ...
    for st in S + tail:
        ...
        # 출력 자리는 AOUT 이 기본이지만, **Q6.9 명령어는 softmax 출력이 OSTR**
        # 이 가리키는 SM 영역입니다 (AOUT 은 bias_k 워드 주소로 씁니다).
        outs = [st.get('AOUT')]
        if st.get('FMT') == FMT_Q69:
            outs = [st.get('OSTR')]
        for ov in outs:
            if ov is None:
                continue
...
# =============================================================================
# 명령어 워드 인코딩 — `rtl/EvT_Engine.v` 의 디코더와 **한 벌**이어야 합니다
#
#   [ 31: 0] KIND[3:0] FMT[5:4] ACT[7:6] VAR[11:8] FLAG[15:12]
#            SHIFT[21:16] SHIFT2[27:22]
...
#         [3] 활성함수 뒤 2차 재양자화 (곱수는 GELU 스칼라와 같은 칸, SHIFT2)
# =============================================================================
...
def set_ostr(S):
    """OSTR(출력 stride) 기본값 = NOUT. 다른 명령어가 명시적으로 덮어씁니다.

    head-major 명령어는 head 간 간격을 넣습니다.
    """
    for st in S:
        if st['kind'] != OP_GEMM:
            continue
...
def annotate(S):
    ...
    손으로 명령어마다 적으면 반드시 어긋나므로, **어떤 명령어가 토큰 수에 의존하는가**
    라는 규칙 하나로 일괄 결정합니다.
    """
    for st in S:
        nm = st['name']
        var = flag = 0
        # 토큰 행을 도는 명령어는 M 이 n_tok
...
def annotate2(S, CFG):
    ...
    (`RQ_BASE + NOUT`), shift 는 여기서 붙입니다. `linear1` 은 뒤가 LayerNorm 이라
    int8 격자가 없어 **Q4.11 16비트를 그대로** 씁니다(raw16).
    """
    GELU_SH, RAW16, REQ2 = CFG['gelu_shift'], set(CFG['gelu_raw16']), set(CFG['req2'])
    SH2, RQB2 = CFG['shifts'], CFG['rq_base']

    # 본체 GEMM 은 `gemm()` 헬퍼가 manifest 에서 상수를 채우지만 **꼬리는 dict 를
    # 직접 만들어** 그 경로를 안 탑니다. 그래서 `SHIFT=None, RQ_BASE=None` 이 그대로
    ...
    for st in S:
        lay = st.get('layer')
        if st['kind'] in (OP_GEMM, OP_ARGMAX) and lay in RQB2:
            if st.get('RQ_BASE') is None:
                st['RQ_BASE'] = RQB2[lay]
            if st.get('SHIFT') is None:
                st['SHIFT'] = SH2.get(lay) or 0
    ISCALE = CFG['input_scales']
    for st in S:
        if st['kind'] == OP_GEMM and st.get('FMT') == FMT_Q411:
            lay = st['layer']
            if lay in RAW16:
                st['FLAG'] = (st.get('FLAG') or 0) | F_RAW16
                st['SHIFT2'] = 0
            else:
                st['SHIFT2'] = GELU_SH[lay]
        if st['kind'] == OP_GEMM and st.get('layer') in REQ2:
            st['FLAG2'] = (st.get('FLAG2') or 0) | F2_REQ2
            st['SHIFT2'] = GELU_SH[st['layer']]
        # `layer_norm_2` 만 입력이 정수 코드(Q4.11) 입니다 — 나머지는 bf16
        if st['kind'] == OP_LN and st['layer'].endswith('layer_norm_2'):
        ...
        if st['name'] == 'proc_ev.res':
            ...
            # scale 을 `round(scale * 2^RSH)` 정수로 (RTL 의 RSH 와 한 벌).
            # bf16 상수로 두면 비율이 0.4 % 흔들려 결과가 절반 어긋납니다.
            st['RQ_BASE']   = fx_scale(ISCALE['backbone.proc_event_blocks.0.seq_init.1'])
            st['OSTR'] = fx_scale(SEQ5_SCALE)


def enc(st):
    """명령어 dict → 256비트 정수"""
    kind = st['kind']
    cons = st.get('FMT') or 0
    act  = st.get('ACT') or 0
    var  = st.get('VAR') or 0
    flag = st.get('FLAG') or 0
    sh   = st.get('SHIFT') or 0
    gsh  = st.get('SHIFT2') or 0
    ...
    ws = [w0, st.get('M') or 0, st.get('K') or 0, st.get('NOUT') or 0,
          ...
          ((st.get('RQ_BASE') or 0) & 0xFFFF) | (((st.get('OSTR') or 0) & 0xFFFF) << 16)]
    v = 0
...
def emit_insts(dst, S, tail):
    words = [enc(x) for x in (S + tail)]
    with open(os.path.join(dst, 'instmem.bin'), 'wb') as fb, \
         open(os.path.join(dst, 'instmem.hex'), 'w') as fh:
        for v in words:
...
def main():
    ...
    n_inst = emit_insts(args.dst, S, tail)
    print(f'명령어 이미지 {n_inst} 워드 → instmem.bin (본체 {len(S)}, 끝 {len(tail)})')
