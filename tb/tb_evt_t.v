...
//     A_Mem 을 넘습니다)
//   · `n_tok` 이 타임스텝마다 바뀌는데 명령어 프로그램은 **그대로** 라는 것
...
module tb_evt_t;
  ...
  integer fed = 0;
  always @(posedge clk) if (!rst && tok_req && fed <= tstep_idx) begin
    fed = tstep_idx + 1;
    $display("  t=%0d 요청 → 토큰 %0d개 적재 (%0d 사이클째)", tstep_idx,
             ntok[tstep_idx], cyc);
    feed(tstep_idx);
  end
  ...
  initial begin
    ...
    if (cyc >= 100000000) begin
      $display("=== TIMEOUT state=%0d inst=%0d t=%0d ===",
               dbg_state, dbg_inst, tstep_idx);
      $finish;
    end
