...
module tb_evt_compile;
  ...
  reg start=0;
  wire done, busy; wire [3:0] dbg_state; wire [AW_INST-1:0] dbg_inst;
  reg [AW_INST-1:0] n_body=4, n_tail=1; reg [5:0] n_tstep=2;
  reg [31:0] eps=32'h3727c5ac;
  wire [5:0] tstep_idx; reg [DIM_W-1:0] tok_n=32;
  reg ld_we=0; reg [2:0] ld_sel=0; reg [AW_W-1:0] ld_addr=0; reg [N*16-1:0] ld_data=0;
  wire [3:0] res_class; wire [10*32-1:0] res_logits;
  ...
  initial begin
    ...
    @(negedge clk); start=0;
    if (done) $display("  기동 → done  %0d 사이클 (inst %0d, state %0d)", cyc, dbg_inst, dbg_state);
    else      $display("  [진행 중] %0d 사이클에서 state=%0d inst=%0d — 기능 검증은 tb_evt 에서",
                       cyc, dbg_state, dbg_inst);
