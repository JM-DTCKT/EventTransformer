...
def test_int8_linear():
    ...
    # what the fp32-bias path used to compute, for the error bound below
    y_fp32 = hw_quant.int8_linear(x, x_scale, w_int, ws) + b
    ...
    check('folded bias vs adding it in fp32: <= half a step (+1 fp32 ulp)',
...
def test_payload_roundtrip():
    ...
    w_scale = hw_quant.compute_weight_scale(w, 8)
    packed = hw_quant._pack_scale(w_scale.reshape(-1))
    ...
    check('weight = q * scale exactly', torch.equal(recon, q.float() * s.reshape(-1, 1)))
    rel = ((recon - w).abs() / w.abs().clamp(min=1e-6))
    check('INT8 weight error stays small',
          float(rel.median()) < 0.02, f'median rel err {float(rel.median()):.4f}')
    check('scale codes are int16 (no fp32 scale survives)',
          packed['codes'].dtype == torch.int16)
