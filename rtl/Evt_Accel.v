...
//
//  1. **step 을 레지스터로 안 넣습니다.** `fpga_nl` 은 step 12개라 AXI-Lite
//     레지스터 파일(12x8워드)에 담았지만 여기는 122 step x 8워드 = 976 쓰기라
//     Step_Mem 을 DMA 목적지(sel 4)로 넣습니다.
//
//  2. **타임스텝마다 입력을 새로 받습니다.** X/PIN 은 A_Mem 에 한 타임스텝분만
//     들어갑니다 (20벌이면 24k 워드로 A_Mem 을 넘습니다). 엔진이 `tok_req` 로
//     멈추면 PS 가 그 타임스텝의 X/pos enc 를 DMA 하고 `TOK_ACK` 를 씁니다.
//
//  3. **결과가 클래스 하나**입니다. 확률 벡터가 아니라 argmax 이므로 레지스터
//     하나로 끝나고, 로짓 10개는 디버그용으로 함께 노출합니다.
//
// =============================== 레지스터 맵 =================================
//  off    R/W  이름          내용
//  0x000   W   CTRL          [0] start(펄스)  [1] dump(펄스)
//  0x004   R   STATUS        [0] done  [1] busy  [5:2] state  [13:6] step
//                            [14] tok_req  [15] loader busy  [16] dumping
//                            [22:17] 요청 중인 타임스텝 t
//  0x008  RW   N_BODY        타임스텝당 step 수 (117)
//  0x00C  RW   N_TAIL        끝에 한 번 도는 step 수 (5)
//  0x010  RW   N_TIME        타임스텝 수 T (<= 20)
//  0x014  RW   LOAD_SEL      0=W 1=A 2=PB 3=PG 4=Step
...
//
// ------------------------------- 사용 순서 -----------------------------------
//   1회   : W / PB / PG / Step 을 DMA (LOAD_SEL → LOAD_BASE → DMA)
...
module Evt_Accel #(
    ...
    localparam RES_LO = 10'd256;                    // 0x400
    ...
    reg [AW_S-1:0]  r_n_body, r_n_tail;
    reg [5:0]       r_n_time;
    reg [2:0]       r_load_sel;
        ...
        if (rst) begin
            ...
            if (wr_fire) begin
                ...
                case (wa)
                    ...
                    10'h002: r_n_body    <= s_axi_wdata[AW_S-1:0];
                    10'h003: r_n_tail    <= s_axi_wdata[AW_S-1:0];
                    10'h004: r_n_time    <= s_axi_wdata[5:0];
    ...
    wire [3:0]      eng_state;
    wire [AW_S-1:0] eng_step;
    wire [5:0]      tok_idx;
    wire            tok_req;
    wire [3:0]      res_class;
    wire [N*PSUM_W-1:0] res_logits;
    ...
    wire                dmp_rd_en, dmp_busy;
    wire [AW_A-1:0]     dmp_rd_addr;
    wire [N*16-1:0]     dmp_rd_data;
    ...
    EvT_Engine #(.N(N), .ACT_W(ACT_W), .PSUM_W(PSUM_W), .DIM_W(DIM_W),
        ...
        .n_body(r_n_body), .n_tail(r_n_tail), .n_time(r_n_time), .eps(r_eps),
        .tok_rd_idx(tok_idx), .tok_rd_n(r_tok_n),
        ...
        .dbg_rd_en(dmp_rd_en), .dbg_rd_addr(dmp_rd_addr),
        .dbg_rd_data(dmp_rd_data));
    ...
    Axis_Dump #(.SW(SW), .DW(N*16), .AW(AW_A)) u_dm (
        .clk(aclk), .rst(rst),
        .start(r_dump), .base(r_dump_base), .len(r_dump_len),
        .rd_en(dmp_rd_en), .rd_addr(dmp_rd_addr), .rd_data(dmp_rd_data),
    ...
    // 사이클 카운터 + done 래치
    // =========================================================================
    // 엔진의 `done` 은 `start` 가 내려가면 곧바로 IDLE 로 가며 사라집니다.
    // AXI 폴링은 그 한 순간을 놓치기 쉬우므로 **붙잡아 둡니다** (start 에서 해제).
    // `fpga_nl` 통합 TB 에서 `fin` 래치가 필요했던 것과 같은 이유입니다.
    reg [31:0] cyc_run, cyc_last;
    reg        done_l;
    always @(posedge aclk) begin
        if (rst) begin cyc_run <= 32'd0; cyc_last <= 32'd0; done_l <= 1'b0; end
        else begin
            if (r_start) begin cyc_run <= 32'd0; done_l <= 1'b0; end
            else if (eng_busy) cyc_run <= cyc_run + 1'b1;
            // `done` 은 **busy 가 이미 내려간 뒤** 한 사이클만 뜹니다 — 엔진이
            // S_DONE 에서 `done<=1` 과 `st<=S_IDLE` 을 같은 엣지에 하기 때문입니다.
            // busy 로 게이팅하면 영영 못 잡습니다.
            if (eng_done && !done_l) begin cyc_last <= cyc_run; done_l <= 1'b1; end
        end
    end
    ...
    // 읽기
    // =========================================================================
    wire [31:0] status = {9'd0, tok_idx, dmp_busy, ld_busy, tok_req,
                          eng_step, eng_state, eng_busy, done_l};
        ...
        if (rst) begin
            ...
            if (rd_fire) begin
                ...
                case (ra)
                    ...
                    10'h002: s_axi_rdata <= {{(32-AW_S){1'b0}}, r_n_body};
                    10'h003: s_axi_rdata <= {{(32-AW_S){1'b0}}, r_n_tail};
                    10'h004: s_axi_rdata <= {26'd0, r_n_time};
                    ...
                    default: s_axi_rdata <= (ra >= RES_LO && ra < RES_LO + 10)
