...
//
// `Requant_Col` 안에 ReLU 를 인라인해 두었던 것을 모듈로 뺐습니다. 레이어마다
// 런타임으로 고를 수 있어야 하므로(마지막 레이어는 활성함수가 없음) 파라미터가
// 아니라 **입력 포트**로 선택합니다.
//
//   act_sel                       act_parm         회로
//   ------------------------------------------------------------------------
...
// ## 왜 부호비트 마스킹으로 끝나는가
//
// 이 데이터패스의 activation 은 **대칭 양자화**(zero-point = 0)라 실수 0 이 코드 0
// 에 정확히 대응합니다. 따라서 ReLU 에 비교기도, 뺄셈도, zero-point 보정도 필요
...
// ## 여기 없는 것
//
// GELU · softmax · sigmoid 는 int8 코드가 아니라 **Qm.n 고정소수(Q4.11 / Q6.9)** 에서
// LUT 로 계산합니다 (`quantization/hw_flow.md` §2.3, §2.4). 포맷도 자원 규모도 달라
// 이 모듈에 넣지 않았습니다 — 필요해지면 별도 블록으로 붙이는 게 맞습니다.
// -----------------------------------------------------------------------------
module Activation #(
