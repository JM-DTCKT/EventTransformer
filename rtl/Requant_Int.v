...
//
// 파이프라인 2단 (곱 / 반올림+시프트+포화), in_valid → out_valid 2클럭.
// `PIPE_PRE=1` 이면 프리애더를 따로 끊어 **3단** 이 됩니다.
//
// **이 파일은 `fpga_nl` 과 심볼릭 링크로 공유합니다.** 그래서 기본값은 2단이고,
// 3단은 쓰는 쪽에서 `.PIPE_PRE(1)` 로 켭니다 (`fpga_dvs128_10` 이 켭니다).
//
// 프리애더를 곱과 같은 사이클에 두면 `acc(32b) + bias(32b)` 의 CARRY 체인이
// 곱셈기 앞에 그대로 붙습니다. acc 가 32비트라 DSP48E2 의 27비트 pre-adder 에
// 안 들어가고 **패브릭 가산기**로 합성되기 때문입니다 (아래 [주의] 참조).
// ZU9EG -2 에서 이 경로가 WNS -0.811 ns 로 실패해 한 단을 끊었습니다.
// `mult`/`shift` 도 같이 등록해 레인 안으로 끌어들입니다 — 상수 하나가
// DSP 64개를 먹던 팬아웃이 레인별 FF 로 나뉩니다.
//
// folded INT32 bias (USE_BIAS=1):
//       acc[c] += b_int[c] 를 곱 앞에서 수행합니다. `(acc + b_int) * M` 이
//       DSP48E2 의 **27비트 pre-adder**(P = (A+D)*B) 에 매핑되기를 기대했지만,
//       acc 를 32비트로 두면 포트에 안 들어가 패브릭 가산기가 됩니다 —
//       그래서 위처럼 한 단을 끊어 곱셈 경로에서 떼어냅니다. b_int[c] 와 M[c] 는 같은 채널
//       인덱스라 ROM 주소생성기도 공유합니다.
//       (Mac_OS 의 Linear_Top 은 bias 포트가 없으므로 여기가 bias 를 더하는 자리)
//
// 주의: acc(23b 유효) × M(31b) = 54비트라 DSP48E2 의 48비트 P 를 넘습니다.
...
module Requant_Int #(
    ...
    // `(acc + b_int) * M` 은 DSP48E2 의 **27비트 pre-adder**(P = (A+D)*B)에 그대로
    // 매핑됩니다. acc 는 실측 23b, b_int 는 14b 라 둘 다 27b 포트에 들어가므로
    // bias 덧셈에 드는 추가 로직이 **없습니다**. USE_BIAS=0 이면 프리애더를 빼고
    // 합성됩니다.
