...
//
// LayerNorm 으로 들어가는 6개 GEMM(out_proj x3, linear3 x3)이 쓰는 경로입니다.
// LayerNorm 입력이 bf16 이므로 그 생산자는 Qm.n 격자로 스냅하지 않고 bf16 으로
// 변환합니다 — 정수 시프트도, 포화도 없습니다.
//
// **반올림은 한 번만** 합니다. `Int32_To_Bf16` 로 먼저 줄인 뒤 곱하면 acc 의
// 하위 비트를 곱하기도 전에 버립니다. 여기서는 정수 누산기와 scale 의 가수를
// **정확히** 곱한 뒤(ACC_W+8 비트) 한 번만 bf16 으로 반올림합니다
...
//
// [타이밍] 한 사이클에 두면 `|acc| → 41b 곱 → 41b LZC → 41b 배럴 시프트` 가
// 직렬로 놓입니다. 자르는 자리가 곱 **뒤**인 것은 이 모듈의 무게가 뒤쪽
// (LZC + 정규화)에 실려 있기 때문입니다 — 앞을 끊는 `Requant_Int` 의
// `PIPE_PRE` 와 반대입니다.
//
// 2단이므로 `EvT_Engine` 의 **`FCA_LAT_BF16` 도 2** 여야 합니다 (쓰기 주소
// 파이프가 어긋나면 결과가 한 칸 앞 주소에 써집니다).
