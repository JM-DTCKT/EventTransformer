// ============================================================================
//  bf16_to_fix.v  --  BFloat16 -> signed 고정소수점 Q(IW-1-IF).IF 변환 (조합)
// ----------------------------------------------------------------------------
...
//
//  [xsh]  LayerNorm 출력은 입력 스케일에 불변(LN(a*x) = LN(x)) 이므로 xsh 로
//         고정소수점 창(window)을 통째로 옮겨도 결과가 바뀌지 않는다.
//         활성값 분포가 기본 Q8.15 창을 벗어날 때 host 가 조절하는 용도다.
//
//  [경계]  IW=24, IF=15 기준
