// -----------------------------------------------------------------------------
// PE_OS_Pp : `Mac_OS/PE_OS` 에 **ping-pong 누산기**를 붙인 판 (연산은 동일)
//
// 원본은 누산기가 DSP48E2 의 P 하나뿐이라, 컬럼을 다 뽑아낼 때까지 다음 타일을
// 시작할 수 없었습니다. 타일 주기 K+114 중 실제 곱셈은 K 뿐이었습니다.
//
//   P       : 지금 타일을 누적                 (ping)
//   shadow  : 직전 타일의 최종값을 들고 있음   (pong)  ← 읽어내기는 여기서
//
// 둘이 번갈아 역할을 바꾸므로 **읽기와 계산이 완전히 분리**됩니다.
//
...
//     ⇒ PE[i][j] 확정 = K + i + j + 2
//
// 그래서 snapshot 도 clr 도 **전역이면 안 됩니다** — 전역이면 가장 늦게 끝나는
// PE[31][31](K+64)를 모두가 기다려야 하고, 그게 순수 ping-pong 이 K+67 에서
// 멈추는 이유입니다. 둘 다 A 와 같은 방향으로 흘리면 PE 마다 자기 시각에 걸려
// 타일 주기가 **K+4** 까지 내려갑니다.
//
//     snap_in : shadow <= P   (그 PE 가 막 확정된 순간)
//     clr_in  : P <= 0        (그 다음 사이클, 새 타일 데이터 도착 직전)
//
// **DSP48E2 인스턴스는 원본 그대로입니다** — 리셋 소스만 전역 `clr` 에서 이 PE 의
// `clr_in` 으로 바뀝니다. 검증된 매핑(AREG/BREG=0, MREG=1, PREG=1, OPMODE 상수)을
// 건드리지 않는 것이 요점입니다.
//
...
module PE_OS_Pp #(
    ...
    // ---- 아래 곱셈·누산은 `Mac_OS/PE_OS` 원본과 동일 (clr → clr_in) ----
    wire rc = rst | clr_in;
