// -----------------------------------------------------------------------------
// Fp32_Add : IEEE-754 단정밀도 덧셈, round-to-nearest-even
//
// 잔차(residual) 경로의 bf16 덧셈에 씁니다 — `EvT_Engine` 의 RES 명령어.
//
// ## 왜 bf16 이 아니라 fp32 로 누산하는가
//
// bf16 은 가수가 8비트뿐이라 여러 개를 bf16 으로 이어 더하면 오차가 누적됩니다.
// 그래서 누산만 fp32 로 하고 **마지막에 한 번만** bf16 으로 내립니다.
//
// ## 순서가 정의되어 있어야 합니다
//
// 부동소수 덧셈은 결합법칙이 성립하지 않으므로 **RTL 과 골든이 같은 순서**여야
// 비트 단위로 맞습니다.
//
// `x = bf16(x + h)` 는 bf16→fp32 확장(가수 0 채움, 공짜) → fp32 덧셈 → bf16
// 반올림입니다. 골든이 정확히 그렇게 정의돼 있습니다.
//
// ## 구현
//
//   ① 언팩 / 크기 비교 / 지수 정렬 (guard·round·sticky 3비트)
//   ② 가감산 → 정규화(LZC) → RNE 반올림 → 팩
//
// denormal 은 0 으로 플러시합니다 (다른 bf16 블록과 같은 정책). Inf/NaN 은 그대로
// 통과시킵니다 — 이 데이터패스엔 오지 않지만 시뮬에서 X 가 안 번지게 합니다.
//
// 파이프라인 2단, 처리율 1/사이클.
// -----------------------------------------------------------------------------
`timescale 1ns/1ps

module Fp32_Add (
    input  wire        clk,
    input  wire        rst,
    input  wire        in_valid,
    input  wire [31:0] a,
    input  wire [31:0] b,
    output reg         out_valid,
    output reg  [31:0] y
);
    // ---------------- 언팩 ----------------
    wire        sa = a[31],      sb = b[31];
    wire [7:0]  ea = a[30:23],   eb = b[30:23];
    wire [22:0] ma = a[22:0],    mb = b[22:0];

    wire a_zero = (ea == 8'd0);                 // denormal 포함 → 0
    wire b_zero = (eb == 8'd0);
    wire a_spec = (ea == 8'hFF);                // Inf / NaN
    wire b_spec = (eb == 8'hFF);

    // 크기 비교 (지수+가수를 한 덩어리로)
    wire a_big = ({ea, ma} >= {eb, mb});

    wire        s_big = a_big ? sa : sb,   s_sml = a_big ? sb : sa;
    wire [7:0]  e_big = a_big ? ea : eb,   e_sml = a_big ? eb : ea;
    wire [22:0] m_big = a_big ? ma : mb,   m_sml = a_big ? mb : ma;
    wire        z_sml = a_big ? b_zero : a_zero;  // 작은 수가 0인지 나타내는 bit

    // 24비트 유효숫자를 3비트(Guard,Round,Sticky) 자리만큼 올려 27비트로, 
    wire [26:0] sig_big = {1'b1, m_big, 3'b000};
    wire [26:0] sig_sml = z_sml ? 27'd0 : {1'b1, m_sml, 3'b000};

    wire [7:0]  d = e_big - e_sml;

    // ---------------- 정렬 (sticky 보존) ----------------
    reg  [26:0] sml_sh;
    reg         sticky;
    integer     i;
    always @* begin
        if (d >= 8'd27) begin
            sml_sh = 27'd0;
            sticky = |sig_sml;
        end else begin
            sml_sh = sig_sml >> d;
            sticky = |(sig_sml & ((27'd1 << d) - 27'd1)); // 밀려난 하위 d bit 중 하나라도 1이 있는지 나타내는 bit
        end
    end
    wire [26:0] sml_al = sml_sh | {26'd0, sticky};

    wire same_sign = (s_big == s_sml);
    wire [27:0] sum = same_sign ? ({1'b0, sig_big} + {1'b0, sml_al})
                                : ({1'b0, sig_big} - {1'b0, sml_al});

    // ---------------- stage 1 ----------------
    reg         v1, s1, spec1, bigzero1;
    reg [31:0]  spec_val1;
    reg [7:0]   e1;
    reg [27:0]  sum1;

    always @(posedge clk) begin
        if (rst) begin
            v1 <= 1'b0; spec1 <= 1'b0;
        end else begin
            v1   <= in_valid;
            s1   <= s_big;
            e1   <= e_big;
            sum1 <= sum;
            // 특수값: Inf/NaN 이 하나라도 있으면 그걸 그대로.  둘 다 0 이면 +0.
            spec1     <= a_spec | b_spec | (a_zero & b_zero);
            spec_val1 <= a_spec ? a : (b_spec ? b : 32'd0);
            bigzero1  <= a_zero & b_zero;
        end
    end

    // ---------------- 정규화 : leading-zero count ----------------
    // 선행 1 을 **bit 26** 으로 올리는 시프트량입니다 (mant 가 norm[26:3] 이므로).
    // 자리올림(bit 27)은 아래에서 따로 처리하므로 여기서는 26..0 만 훑습니다.
    //
    // [타이밍] 예전에는 26→0 을 훑으며 `lz_found` 로 앞선 비트를 막는 **선형
    // 우선순위 인코더**였습니다. `lz_found` 가 직렬 의존이라 27 단 리플이 되고,
    // 그 뒤의 배럴 시프트·지수 보정·`zero_res` 까지 이어져
    // `sum1 -> y_reg` 가 **20 로직단 / 5.045 ns** 가 됐습니다 (실측 WNS -0.101,
    // 32 레인이 전부 같은 모양이라 실패가 무더기로 나옵니다).
    //
    // 같은 값을 **이진 탐색 트리**로 구합니다 — 16/8/4/2/1 로 반씩 줄이며
    // 5 단이면 끝나고, 리덕션 OR 도 LUT6 한 단씩입니다. 레지스터는 안 늘고
    // (조합 논리 교체라 **사이클 수 불변**) LUT 는 오히려 줄어듭니다.
    //
    // sum1[26] 이 bit31 에 오도록 5 칸 밀어 두면, 앞의 0 개수가 곧 `lz` 입니다.
    // 아래 5 비트가 항상 0 이라 sum1[26:0] != 0 이면 결과는 26 이하입니다.
    wire [31:0] lzv = {sum1[26:0], 5'b0};
    wire        l4  = ~|lzv[31:16];
    wire [15:0] lq4 = l4 ? lzv[15:0] : lzv[31:16];
    wire        l3  = ~|lq4[15:8];
    wire [7:0]  lq3 = l3 ? lq4[7:0]  : lq4[15:8];
    wire        l2  = ~|lq3[7:4];
    wire [3:0]  lq2 = l2 ? lq3[3:0]  : lq3[7:4];
    wire        l1  = ~|lq2[3:2];
    wire [1:0]  lq1 = l1 ? lq2[1:0]  : lq2[3:2];
    wire        l0  = ~lq1[1];
    // sum1[26:0] == 0 이면 트리가 31 을 내므로 예전과 같이 0 으로 맞춥니다
    // (그 경우 `zero_res` 가 서서 y = 0 이라 값 자체는 안 쓰입니다).
    wire [4:0] lz = (lzv == 32'd0) ? 5'd0 : {l4, l3, l2, l1, l0};

    reg  [27:0] norm;
    reg  [8:0]  e_norm;
    always @* begin
        if (sum1[27]) begin                           // 자리올림 → 오른쪽 1
            norm   = {1'b0, sum1[27:1]} | {27'd0, sum1[0]};   // sticky 유지
            e_norm = {1'b0, e1} + 9'd1;
        end else begin
            norm   = sum1 << lz;                      // 선행 1 을 bit26 으로
            e_norm = {1'b0, e1} - {4'd0, lz};
        end
    end

    // ---------------- RNE 반올림 ----------------
    wire [23:0] mant  = norm[26:3];                   // 1.xxx (24비트)
    wire        g     = norm[2], r = norm[1], st = norm[0];
    wire        up    = g & (r | st | mant[0]);
    wire [24:0] mant_r = {1'b0, mant} + {24'd0, up};
    wire        carry  = mant_r[24];
    wire [23:0] mant_f = carry ? mant_r[24:1] : mant_r[23:0];
    wire [8:0]  e_fin  = e_norm + {8'd0, carry};

    wire zero_res = (sum1 == 28'd0) || (e_fin[8] == 1'b1) || (e_fin == 9'd0);
    wire ovf      = (e_fin >= 9'd255) && !e_fin[8];

    always @(posedge clk) begin
        if (rst) begin
            out_valid <= 1'b0; y <= 32'd0;
        end else begin
            out_valid <= v1;
            if (v1) begin
                if (spec1)        y <= bigzero1 ? 32'd0 : spec_val1;
                else if (ovf)     y <= {s1, 8'hFF, 23'd0};        // Inf
                else if (zero_res) y <= 32'd0;                     // 언더플로 → +0
                else              y <= {s1, e_fin[7:0], mant_f[22:0]};
            end
        end
    end
endmodule
